...
class Optimus_Env(Scheduler):
    ...
    def est_util(self, job) -> (float, str):
    ...
    def _schedule(self):
        ...
        for job in self.uncompleted_jobs:
            ...
            opt_queue.put((util, random(), job, role))

        while not opt_queue.empty():
            utility, _, job, role = opt_queue.get()
            if utility >= 0:
                break
        ...
        for job in self.uncompleted_jobs:
            self.logger.debug(self.name + ":: scheduling results" + " type: " + str(job.type) + " num_worker: " + str(
                job.num_workers) + " num_ps: " + str(job.num_ps))
