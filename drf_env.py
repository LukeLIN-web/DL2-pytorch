...
class DRF_Env(Scheduler):
    ...
    def _schedule(self):
        ...
        for job in self.uncompleted_jobs:
            drf_queue.put((0, job.arrv_time, random.random(), job))  # enqueue jobs into drf queue

        while not drf_queue.empty():
            (_, job_arrival, _, job) = drf_queue.get()
            ...
            if succ:
                ...
                if job.num_workers < pm.MAX_NUM_WORKERS and job.num_ps < pm.MAX_NUM_WORKERS:
                    drf_queue.put((job.dom_share, job_arrival, random.random(), job))
