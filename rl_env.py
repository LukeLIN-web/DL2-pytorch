                            ...
                            if job:
                                if pm.BUNDLE_ACTION:
                                    ...
                                    if min(self.action_freq) > 0 and \
                                        ...
                                        if mask[3 * i + index] > 0 and \
                                                masked_output[0][3 * i + index] > pm.MIN_ACTION_PROB_FOR_SKIP:
                                            action = 3 * i + index
                                    ...
                                    if (job.num_workers == 0
                                        ...
                                        if mask[3 * i + 2] > 0 and \
                                                ...
                                                masked_output[0][3 * i] > pm.MIN_ACTION_PROB_FOR_SKIP:
                                            if np.random.rand() < 0.5:
                                                action = 3 * i + 2  # increase this job's bundle
                                                self.logger.debug("Got 2.")
                                            else:
                                                action = 3 * i
                                                self.logger.debug("Got 2.")
                                            break
                                    elif job.num_workers >= job.num_ps * 0.5 and \
                                            np.random.rand() < 0.5:
                                        if mask[3 * i + 2] > 0 and \
                                                masked_output[0][3 * i + 2] > pm.MIN_ACTION_PROB_FOR_SKIP and \
                                                mask[3 * i + 1] > 0 and \
                                                masked_output[0][3 * i + 1] > pm.MIN_ACTION_PROB_FOR_SKIP:
                                            if np.random.rand() < 0.01:
                                                action = 3 * i + 2  # increase this job's bundle
                                                self.logger.debug("Got 3.")
                                            else:
                                                action = 3 * i + 1  # increase ps
                                                self.logger.debug("Got 4.")
                                            break
                                else:
                                    if job.num_workers == 0 and \
                                            mask[2 * i] > 0 and \
                                            masked_output[0][2 * i] > pm.MIN_ACTION_PROB_FOR_SKIP and \
                                            np.random.rand() < 0.01:
                                        action = 2 * i
                                        self.logger.debug("Got 1.")
                                        break
                                    elif job.num_ps == 0 and \
                                            mask[2 * i + 1] > 0 and \
                                            masked_output[0][2 * i + 1] > pm.MIN_ACTION_PROB_FOR_SKIP and \
                                            np.random.rand() < 0.01:
                                        action = 2 * i + 1
                                        self.logger.debug("Got 2.")
                                        break
                                            ...
                                            masked_output[0][2 * i] > pm.MIN_ACTION_PROB_FOR_SKIP and \
                                            np.random.rand() < 0.5:
                                        action = 2 * i  # increase this job's worker
                                        self.logger.debug("Got 3.")
                                        break
                                            ...
                                            masked_output[0][2 * i + 1] > pm.MIN_ACTION_PROB_FOR_SKIP and \
                                            np.random.rand() < 0.5:
                                        action = 2 * i + 1  # increase this job's ps
                                        self.logger.debug("Got 4.")
                                        break
            else:
                #  inference mode, no training mode
...
def test():
    import log, trace
    logger = log.getLogger(name="agent_", level="INFO")
    job_trace = trace.Trace(logger).get_trace()
    env = RL_Env("RL", job_trace, logger)
    env._prepare()
    env.observe()
    output = np.random.rand(1, 61)  # demo, just for this test.
    while not env.end:
        data = env.step(output)
        for item in data:
            print(item)
        print("-----------------------------")
